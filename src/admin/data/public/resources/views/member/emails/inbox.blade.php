    ...
    <div class="card">
        <div class="card-header">
            ...
            <div class="card-header-form">
                <form  method="GET" action="{{ route('member.emails.inbox') }}" >
        ...
        <form id="msg-list" action="{{ route('member.email.delete-multiple-inbox') }}" onsubmit="return confirm('@lang('admin.delete-prompt')')" method="post">
            @csrf
                        ...
                        @foreach($emails as $item)
                            <tr @if($item->pivot->read==0) class="unread" @endif >
                                ...
                                <td>
                                    ...
                                    &nbsp;
                                    <a href="{{ route('email.view-inbox',['email'=>$item->id]) }}" class="wide-link"  >
                                ...
                                <td  onclick="document.location.replace('{{ route('member.email.view-inbox',['email'=>$item->id]) }}')"  class="clickable">
                                    <a  class="wide-link" href="{{ route('member.email.view-inbox',['email'=>$item->id]) }}">{{ $item->subject }}</a>
                                </td>
                                <td  onclick="document.location.replace('{{ route('member.email.view-inbox',['email'=>$item->id]) }}')"  class="clickable">
                                ...
                                <td  onclick="document.location.replace('{{ route('member.email.view-inbox',['email'=>$item->id]) }}')"  class="clickable">{{ \Illuminate\Support\Carbon::parse($item->crated_at)->format('D, M d, Y') }}</td>
                                <td>
                                    <a  href="{{ route('member.email.delete-inbox',['id'=>$item->id]) }}"  title="@lang('site.delete')" onclick="return confirm('@lang('admin.delete-prompt')')" ><i class="fa fa-trash"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody></table>
