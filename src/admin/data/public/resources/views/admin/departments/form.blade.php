<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 form-group {{ $errors->has('church_branch') ? 'has-error' : ''}}">
        <label for="church_id" class="control-label">{{ 'Church branch' }}</label>
   
        <select required  name="church_id" class="form-control" id="church_id" required>
         @foreach(\App\Church::get() as $church)
            <option value="{{ $church->id }}" {{ ((null !== old('church_id',@$department->church_id)) && old('church_id',@$department->church_id) == $church->id) ? 'selected' : ''}}>{{ $church->name }}</option>
         @endforeach 
        </select>
    {!! $errors->first('church_branch', '<p class="help-block">:message</p>') !!}
</div>

<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 form-group {{ $errors->has('church_branch') ? 'has-error' : ''}}">
    <label for="church_branches" class="control-label">{{ 'Group Type' }}</label>
